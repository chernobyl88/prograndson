...
class codeBarreC128 {
	...
	public function setOrientation($orientation) {
		...
		if( $orientation == 'h' or $orientation == 'horizontal' ) {
			...
			if (\Library\Application::appConfig()->getConst("LOG"))
				throw new \Exception("Error ID: " . \Library\Application::logger()->log("Error", "codeC128", self::ERROR_UNKNOWN_ORIENTATION, __FILE__, __LINE__));
			else
	...
	public function setFormat($format) {
		...
		if( in_array($format, $this->authorizedFormat) ) {
			...
			if (\Library\Application::appConfig()->getConst("LOG"))
				throw new \Exception("Error ID: " . \Library\Application::logger()->log("Error", "codeC128", self::ERROR_UNAUTHORIZED_FORMAT, __FILE__, __LINE__));
			else
	...
	public function Output($fileName = false) {
		...
		if( !$fileName ) {
			if( $this->format == 'PNG' ) {		
				...
				imagepng($codeBarre, "../Upload/code128/".$codeBarre); 
			} elseif( $this->format == 'GIF' ) {
				//header('Content-Type: image/gif'); 
				imagegif($codeBarre, "./Web/Upload/code128/".$this->name.".gif"); 
			...
			if( !$generated ) {
				if (\Library\Application::appConfig()->getConst("LOG"))
					throw new \Exception("Error ID: " . \Library\Application::logger()->log("Error", "codeC128", self::ERROR_MAKE_FILE, __FILE__, __LINE__));
				else
